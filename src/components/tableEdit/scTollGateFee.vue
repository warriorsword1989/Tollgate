  ...
  <div class="tableEditPanel" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(243, 239, 239, 0.5);">
    <el-form v-for="(dataItem, index) in dataModels" :key="index" :model="dataItem" ref="dataItem" :inline="true" class="wraper">
      <div class="grid-content">
        ...
        <div class="inputPart">
          <el-input v-model="dataItem.out_fee" size="mini"></el-input>
        </div>
      </div>
      <div class="grid-content">
        <div class="labelText">内埠车牌范围：</div>
        <div class="inputPart">
          <el-input v-model="dataItem.local_plate" size="mini"></el-input>
        </div>
      </div>
      <div class="grid-content">
        <div class="labelText">收费类型：</div>
        <div class="inputPart">
          <el-select size="mini" v-model.number="dataItem.type" placeholder="请选择">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let param = {
        table: 'SC_TOLL_TOLLGATEFEE',
        pid: _self.selectedData.tollgateId
      };
      getTollGate(param)
        .then(result => {
