...
<vue-draggable-resizable
  ...
  <!-- table-->
  <el-table ref="multipleTable" :data="tableData" :max-height="100" tooltip-effect="light">
    ...
    <el-collapse-item name="1">
      ...
      <el-tabs @tab-click='tabOnActive'  :tab-position="tabPosition">
        <el-tab-pane :label="isEdit[0]?'* 客车车型划分及费率':'客车车型划分及费率'">
          <sc-toll-car @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-car>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[1]?'* 货车车型划分及空载费率': '货车车型划分及空载费率'">
          <sc-toll-truck @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-truck>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[2]?'* 货车计重装载费率(不超限)':'货车计重装载费率(不超限)'">
          <sc-toll-load @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-load>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[3]?'* 货车计重装载费率(广东不超限)':'货车计重装载费率(广东不超限)'">
          <sc-toll-load-gd @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-load-gd>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[4]?'* 货车计重装载费率(超限)':'货车计重装载费率(超限)'">
          <sc-toll-over-load @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-over-load>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[5]?'* 收费站加费信息表':'收费站加费信息表'">
          <sc-toll-gate-fee @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-gate-fee>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[6]?'* 收费站区间表': '收费站区间表'">
          <sc-toll-grou @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-grou>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[7]?'* 限制载重信息表' : '限制载重信息表'">
          <sc-toll-limit @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-limit>
        </el-tab-pane>
        <el-tab-pane :label="isEdit[8] ? '* 桥隧道收费附加表': '桥隧道收费附加表'">
          <sc-toll-rdlink-bt @tabStatusChange="changeTabStatus" :selected-data='tableData[0]'></sc-toll-rdlink-bt>
        </el-tab-pane>
      </el-tabs>
    </el-collapse-item>
  </el-collapse>
...
<script>
  ...
  export default {
    name: 'tabDiag',
    props: ['dialogTableVisible','tableData'],
    components: {
      scTollCar,
      scTollGateFee,
      scTollGrou,
    ...
    mounted() {
      let boxWidth = this.$el.clientWidth;
      let boxHeight = this.$el.clientHeight;
      let viewWidth = document.documentElement.clientWidth;
      let viewHeight = document.documentElement.clientHeight;
      this.leftDis = (viewWidth - boxWidth) /2;
      this.topDis = (viewHeight - boxHeight) /2;
