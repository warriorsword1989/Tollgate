<template>
  <vue-draggable-resizable
    ...
    <el-tabs @tab-click="tabOnActive" :tab-position="tabPosition">
      ...
      <!-- 根据currentView动态切换组件 -->
      <component @refreshInfoSource="refreshInfoByChild" @tabStatusChange="changeTabStatus" :currentActive="activeIndex" :is="currentView"></component>
    </el-tabs>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshInfoByChild (data) {
        this.transfromSelectedData(this.originSeleceData, this.tables[this.currentIndex], data);
      },

      /**
        * 根据地图上选择的收费站pid查询收费站的概略信息，包括收费站的名称，pid，和收费类型
       */
      async transfromSelectedData(arr, sourceTable, isBridge) {
        let result = null;
        this.loading = true;
        try {
          const param = { table: 'RD_TOLLGATE_NAME', pid: arr, sourceTable: sourceTable, isBridge: isBridge };
    ...
    mounted() {
      ...
      this.transfromSelectedData(this.$store.state.editSelectedData, 'SC_TOLL_CAR', false);
    },
