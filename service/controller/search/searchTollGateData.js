...
class TollData {
  ...
  async getTollSearchData() {
    ...
    let sql = "SELECT t.pid, tn.name , to_char(rn.geometry.get_wkt()) as geometry FROM RD_TOLLGATE T, RD_TOLLGATE_NAME TN, RD_NODE RN WHERE T.PID = TN.PID AND T.NODE_PID = RN.NODE_PID AND ";
